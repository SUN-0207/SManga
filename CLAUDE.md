...
- **Production lives on a home laptop** (Ubuntu 24.04, `sunny-server`) at `https://smanga.shop`, exposed via Cloudflare Tunnel → caddy → 5-container docker compose (`postgres17 + redis7 + api + frontend + watchtower`). Nightly pg_dump backup runs at 02:30 to `/mnt/hdd/backups/` (30d retention). The offsite Google Drive tier (`gdrive:smanga-backups`) was **DISABLED 2026-06-28** — local HDD only now; re-enable by uncommenting the rclone block in `deploy/home/scripts/backup.sh`.
...
- **Vercel staging tier ALSO retired 2026-06-07** — Plan 9's original architecture reserved Vercel as a staging deployment, but the user opted to kill it entirely. SManga runs as a **single environment** (laptop prod only). No PR previews, no pre-merge URL, no rollback-to-staging path. Any future staging needs to be re-introduced on a different stack. Plan 9 plan/spec doc references to "Vercel staging" (Task 18, components table row 9, OAuth callbacks, emergency rollback section) are STALE — both files now carry an Amendment banner at top.
